...
class Road:
    def __init__(self, world):
        ...
        for i in range(NUM_POINTS*self.num_ctrl_points):                    # Fill the Left point and right point 
            self.pointsLeft.append(vect2d(1000,1000))
            self.pointsRight.append(vect2d(1000,1000))
            self.centerPoints.append(vect2d(1000,1000))

        self.ctrl_points[0].co(0, SPACING)                                  # initialise the first two control_points to be straight
    ...
    def createSegment(self, index):
        ...
        ny = np.array([p2.y, p1.y])                                         # Inverted because scipy increasing the X (in this case the Y)
